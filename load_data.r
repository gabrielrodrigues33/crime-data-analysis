...
df_joined <- df_joined %>% select( num_ocorrencia, flag_status, rubrica, conduta, hora_ocorrencia, tempo_registro_key, tempo_ocorrencia_key, pessoa_key, lugar_key)
write.table(df_joined, 'tmp/ocorrencia_fact.csv', row.names=FALSE, col.names=FALSE, sep=',', na = "")
dbExecute(con, 'COPY lugar (num_ocorrencia, flag_status, rubrica, conduta, hora_ocorrencia, tempo_registro_key, tempo_ocorrencia_key, pessoa_key, lugar_key) FROM \'/app/tmp/ocorrencia_fact.csv\' DELIMITER \',\' CSV;')
